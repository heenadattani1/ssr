...
export class MapComponent implements OnInit, OnDestroy {
  ...
  uiHandler = {
    ...
    isMobileMapRender: MobileDetectionService.isAnyMobile() ? true : false,
  ...
  intializeMapBox(events: FetchRaceList[], filters?: AppliedFilters[]) {
    ...
    if (filters && events.length > 0 && filters.length > 0) {
      bound = new LngLatBounds();
      events.forEach((event) => {
        const latlng = new LngLat(event.lng, event.lat);
        bound.extend(latlng);
      });
    ...
    this.markers.forEach((ele, index) => {
      const marker = new Marker({ color: '#f4792f', scale: 0.6 });
      const route =
      ...
      const popup = new Popup({
        ...
        <div id='${ele.id}${index}' class='race_highlights'>
          <div class='race_event_type'>RAGNAR ${
            ele.eventData.type.toUpperCase() === 'RELAY' ? 'ROAD' : ele.eventData.type.toUpperCase()
          }</div>
      ...
      markerDiv.addEventListener('mouseenter', () => {
        if (!MobileDetectionService.isAnyMobile()) {
  ...
  public selectEventFromList(selectedEvent, willOpen, isMobile) {
    ...
    if (isMobile && MobileDetectionService.isAnyMobile()) {
      this.isMobile = true;
      this.uiHandler.isMobileMapRender = false;
  ...
  public renderSingleEvent(selectedEvent: EventData) {
    ...
    bound.extend(new LngLat(selectedEvent.lng, selectedEvent.lat));
  ...
  public clickOnFilterManager(
    ...
    if (this.uiHandler.sideBarDefault && !MobileDetectionService.isAnyMobile()) {
      this.uiHandler.sideBarDefault = false;
    }
    this.isRaceSelected = false;
    switch (clickedOn) {
  ...
  goBackToListView() {
    ...
    if (MobileDetectionService.isAnyMobile()) {
      ...
      if (!this.uiHandler.isMobileMapRender && MobileDetectionService.isAnyMobile()) {
