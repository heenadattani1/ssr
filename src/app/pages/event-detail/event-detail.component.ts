...
import { DataLayerService, eventDetailRoutes, EventService, MetaTagsService } from 'src/app/@core/utils';
import { raceDataFields } from 'src/app/@core/graphql/graphql';
  ...
  MoveActiveTabOption = () => {
    setTimeout(() => {
      if (window.innerWidth <= 767) {
        ...
        $('.tab-options li a').on('click', function (event) {
          zeroLeftOnly(this, '.tab-options ul', 100);
        });
    ...
    setTimeout(() => {
      if (window.innerWidth <= 767) {
        $(window).scroll((event) => {
          ...
          if (scroll >= $('.main_header').height()) {
            $('.event-detail .tab-options').addClass('sticky');
          } else {
            $('.event-detail .tab-options').removeClass('sticky');
          }
        });
      }
    });
