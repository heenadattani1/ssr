...
import { AddedActivityCopy, VirtualChallengeDetail, VirtualChallengeRunLogs, VirtualChallengeTeam } from 'src/app/@core/interfaces/virtual-challenge.interface';
...
import { DataLayerService, VirtualChallengeSharedDataService, XMomentService } from 'src/app/@core/utils';
...
export class AddRunComponent implements OnInit {
  ...
  deleteRun(activity) {
    ...
    modalRef.result.then(
      (result) => {
        if (result) {
          this.getLogsAndLeaderBoard.next('');
