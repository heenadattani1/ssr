...
import { badgeDays, XMomentService } from 'src/app/@core/utils';
...
export class AddLatestActivityComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.selectedDate = `${current.getFullYear()}/${current.getMonth() + 1}/${current.getDate()} 23:59:59`;
    this.selectedDate = this.xMomentService.convertDateWithTimezone('MDT', this.selectedDate).format();
    ...
    if (moment(this.selectedDate).isAfter(endDate) || moment(this.newDate).isSame(endDate)) {
      ...
      this.selectedDate = `${endDate.year()}/${endDate.month() + 1}/${endDate.date()} 23:59:59`;
      this.selectedDate = this.xMomentService.convertDateWithTimezone('MDT', this.selectedDate).format();
  ...
  onDateSelection(date: NgbDateStruct) {
    this.selectedDate = `${date.year}/${date.month}/${date.day} 23:59:59`;
    this.selectedDate = this.xMomentService.convertDateWithTimezone('MDT', this.selectedDate).format();
  ...
  addActivity() {
    ...
    this.virtualChallengeUtilDataService.addActivity(addRunDetail).subscribe(
      (response: AddActivity & { activityStreak: number }) => {
        ...
        this.componentData.memberDetails.logDate = response.logDate;
        ...
        if (this.componentData.challengeDetails.challengeType === 'COMMUNITY') {
          ...
          this.firebaseService.addTeamFeed(response.challengeTeamId, feedDetails);
